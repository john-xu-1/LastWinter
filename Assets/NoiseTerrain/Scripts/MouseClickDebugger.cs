...
public class MouseClickDebugger : MonoBehaviour
{
    ...
    List<ProceduralMapGenerator.PlatformChunkGraph> platformGraph;
    ...
    List<ProceduralMapGenerator.PlatformChunkGraph> platforms = new List<ProceduralMapGenerator.PlatformChunkGraph>();
    ...
    List<ProceduralMapGenerator.PlatformChunkGraph> chunkGroups = new List<ProceduralMapGenerator.PlatformChunkGraph>();
    ...
    void Update()
    {
        ...
        {
            foreach (ProceduralMapGenerator.PlatformChunkGraph platform in platformGraph)
            {
    ...
    private void HandleMouseClickDebugging()
    {
        ...
        else if (clickFunction == HandleMouseClickFunction.isValidWall)
        {
            if (Input.GetMouseButtonUp(1)) //check right side of wall
            {
                ...
                if (chunks.GetTile(clickTile))
                {
                    Debug.Log ($"Is ValidWall right empty: {WallChunk.IsValidWall(clickTile, generator.jumpHeight, true, generator.RoomChunk)}");
            ...
            else if (Input.GetMouseButtonUp(0)) //check left side of wall
            {
                ...
                if (chunks.GetTile(clickTile))
                {
                    Debug.Log($"Is ValidWall left empty: {WallChunk.IsValidWall(clickTile, generator.jumpHeight, false, generator.RoomChunk)}");
    ...
    private void GenerateConnectedChunkGraphThread()
    {
        ...
        foreach (int platformID in platformIDs)
        {
            ...
            ProceduralMapGenerator.PlatformChunkGraph platformChunkGraph = new ProceduralMapGenerator.PlatformChunkGraph();
    ...
    private void GenerateChunkGraphThread()
    {
        ...
        List<ProceduralMapGenerator.PlatformChunkGraph> platformChunkGraphs = new List<ProceduralMapGenerator.PlatformChunkGraph>();

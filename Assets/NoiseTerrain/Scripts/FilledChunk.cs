    ...
    public class FilledChunk
    {
        ...
        private void SetWallChunks (List<Vector2Int> wallSideTiles, int jumpHeight, bool rightSide)
        {
            ...
            while (toVisit.Count > 0)
            {
                ...
                Vector2Int validWall = FindValidWall(toVisit, jumpHeight);
                breakCounter -= 1;
        ...
        public void SetPlatforms(int jumpHeight)
        {
            ...
            for (int y = 0; y < platformIDs.GetLength(1); y += 1)
            {
                for (int x = 0; x < platformIDs.GetLength(0); x += 1)
                {
                    ...
                    //}
                    if (platformIDs[x, y] > 0 && y + minY > 0) platforms[platformIDs[x, y] - 1].jumpTiles.Add(new Vector2Int(x + minX, y + minY));
                }
                //printMap += "\n";
            }
            //Debug.Log(printMap);
        }
        private Vector2Int FindValidWall(List<Vector2Int> wallTiles, int jumpHeight) 
        {
